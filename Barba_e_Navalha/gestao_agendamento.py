...
# Substitua sua função inserir_agendamento_logica por esta versão completa e corrigida:
...
# Adicione esta nova função ao final do seu arquivo gestao_agendamento.py

def listar_todos_servicos_logica(data):
    """
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        return False, f"Erro inesperado ao listar os serviços: {str(e)}"
    
def listar_todos_horarios_logica(data):
    """
...
# Adicione esta nova função ao seu arquivo gestao_agendamento.py
...
def criar_intervalo_logica(data):
    ...
    try:
        ...
        if not all([barbeiro_id, data_intervalo_str, horario_inicio_str, duracao_minutos]):
            return False, "Dados incompletos para criar o intervalo."
        ...
        for ag_existente in agendamentos_existentes:
            if ag_existente.idservicos and ag_existente.idservicos.duracao:
                ...
                if _check_overlap(intervalo_inicio_dt, intervalo_fim_dt, existente_inicio_dt, existente_fim_dt):
                    return False, "Você já possui um agendamento de cliente neste horário."
        ...
        return True, f"Intervalo das {horario_inicio_str} às {intervalo_fim_dt.strftime('%H:%M')} criado com sucesso."

    except Barbeiro.DoesNotExist:
        return False, "Barbeiro não encontrado."
    except Exception as e:
        traceback.print_exc()
        return False, f"Erro inesperado ao criar intervalo: {str(e)}"
...
def editar_local_barbeiro_logica(data):
    ...
    try:
        ...
        if not barbeiro_id or not local_id:
            return False, "ID do barbeiro e do local são obrigatórios."
        ...
        if local_a_editar.barbeirousuarioid_id != int(barbeiro_id):
            return False, "Permissão negada. Você só pode editar seus próprios locais de trabalho."
        ...
        if updated:
            local_a_editar.save()
            return True, f"Local '{local_a_editar.nome_local}' atualizado com sucesso."
        else:
            return True, "Nenhum dado novo foi fornecido para atualização."

    except Local.DoesNotExist:
        return False, "Local com o ID fornecido não foi encontrado."
    except Exception as e:
        import traceback
        traceback.print_exc()
        return False, f"Erro inesperado ao editar o local: {str(e)}"
