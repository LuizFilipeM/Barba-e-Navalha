# BACK-END DE LOGIN AUTENTICAÇÃO
from django.contrib.auth.hashers import check_password  #ERRO NO IMPORT
...
def cadastro(data):
    ...
    usuario = Usuario(
        email=email,
        senha=make_password(senha), #remover a criptografia
        tipo=tipo
    )
...
def editar_perfil(data):
    try:
        ...
        if not usuario_id:
            return {'status': 'error', 'message': 'ID do usuário não fornecido'}
...
def deletar_perfil(data): #remover dados de todas as tabelas
    try:
