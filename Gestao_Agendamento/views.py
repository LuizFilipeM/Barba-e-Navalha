        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                if cliente_id is None:
                    return JsonResponse({"success": False, "message": "O cliente nao existe no banco de dados."})

                cursor.execute('SELECT "ID" FROM "Barbeiro" WHERE "Nome" = %s', [barbeiro_nome])
                barbeiro_id = cursor.fetchone()
                if barbeiro_id is None:
                    return JsonResponse({"success": False, "message": "O barbeiro nao existe no banco de dados."})
                ...
                if existing_agendamento:
                    return JsonResponse({"success": False, "message": "Já existe um agendamento para este barbeiro nesta data e horário."})

                cursor.execute(
                    'INSERT INTO "Agenda" ("Data", "Hora", "IDBarbeiro", "IDCliente", "IDServicos") VALUES (%s, %s, %s, %s, %s) RETURNING "ID"',
                    [data_agendamento, horario, barbeiro_id[0], cliente_id[0], servico_id])
                agenda_id = cursor.fetchone()[0]
                connection.commit()
                return JsonResponse({"success": True, "message": "Agendamento adicionado com sucesso!"})

        except Exception as e:
            connection.rollback()
            return JsonResponse({"success": False, "message": f"Erro ao inserir agendamento: {e}"})
...
def remover_agendamento(request):
    if request.method == 'POST':
        try:
            ...
            agendamento.delete()
            return JsonResponse({"success": True, "message": "Agendamento removido com sucesso."})
        except Agenda.DoesNotExist:
            return JsonResponse({"success": False, "message": "Agendamento não encontrado."})
        except Exception as e:
...
def atualiza_agendamento(request):
    if request.method == 'POST':
        try:
            ...
            agendamento.save()
            return JsonResponse({"success": True, "message": "Agendamento atualizado com sucesso!"})
        except Agenda.DoesNotExist:
            return JsonResponse({"success": False, "message": "Agendamento não encontrado."})
        except Exception as e:
            return JsonResponse({"success": False, "message": str(e)})
...
def lista_agendamentos(request):
    ...
    agendamentos_data = [{"id": ag.id, "data": ag.data, "hora": ag.hora, "servico": ag.idservicos.nome} for ag in agendamentos]
