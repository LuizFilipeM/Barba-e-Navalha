...
def inserir_agendamento_logica(data):
    try:
        ...
        barbeiro_solicitado_id_str = data.get('barbeiro')
        barbeiro_solicitado_nome_str = Barbeiro.objects.get(id=barbeiro_solicitado_id_str).nome
        ...
        if not cliente: return JsonResponse({"status": False, "msg": f"Cliente '{cliente_nome_str}' nao encontrado."})
            
        barbeiro_solicitado = Barbeiro.objects.filter(nome=barbeiro_solicitado_nome_str).first()
        if not barbeiro_solicitado: return JsonResponse({"status": False, "msg": f"Barbeiro '{barbeiro_solicitado_nome_str}' nao encontrado."})

        local_do_barbeiro = Local.objects.filter(barbeirousuarioid=barbeiro_solicitado).first()
        if not local_do_barbeiro: return JsonResponse({"status": False, "msg": f"Barbeiro '{barbeiro_solicitado_nome_str}' nao possui nenhum local de trabalho."})
        ...
        try:
            ...
            if novo_servico.tempo is None or novo_servico.tempo <= 0:
                return JsonResponse({"status": False, "msg": f"O serviço '{novo_servico.nome}' nao possui uma duração valida."})
            duracao_novo_servico_min = novo_servico.tempo
        except Servicos.DoesNotExist:
            return JsonResponse({"status": False, "msg": f"Serviço '{servico_id_str}' nao encontrado ou nao oferecido no local '{local_do_barbeiro.nome_local}'."})
        except (ValueError, TypeError):
            return JsonResponse({"status": False, "msg": f"ID do serviço '{servico_id_str}' inválido."})
        ...
        for ag_existente in agendamentos_barbeiro_no_dia:
            if ag_existente.idservicos and ag_existente.idservicos.tempo:
                ...
                if _check_overlap(novo_agendamento_inicio_dt, novo_agendamento_fim_dt, existente_inicio_dt, existente_fim_dt):
                    ...
                    return JsonResponse ({"status": False, "msg": f"O barbeiro {barbeiro_solicitado_nome_str} ja possui um agendamento conflitante das {existente_inicio_dt.strftime('%H:%M')} às {existente_fim_dt.strftime('%H:%M')}."})
        ...
        for ag_existente in agendamentos_cliente_no_dia:
            if ag_existente.idservicos and ag_existente.idservicos.tempo:
                ...
                existente_fim_dt = existente_inicio_dt + datetime.timedelta(minutes=ag_existente.idservicos.tempo)
                if _check_overlap(novo_agendamento_inicio_dt, novo_agendamento_fim_dt, existente_inicio_dt, existente_fim_dt):
                    return JsonResponse ({"status": False, "msg": f"O cliente {cliente_nome_str} ja possui um agendamento conflitante das {existente_inicio_dt.strftime('%H:%M')} às {existente_fim_dt.strftime('%H:%M')}."})
        ...
        if not success_calendar:
            return JsonResponse({"status": False, "msg": msg_calendar})
