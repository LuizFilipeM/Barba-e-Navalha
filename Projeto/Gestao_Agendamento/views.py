...
def atualiza_agendamento(data):
    ...
    try:
        ...
        if novo_servico:
            print("ANTES")
            teste = Servicos.objects.get(id=novo_servico)
            agendamento.idservicos = teste
            print(agendamento.idservicos)
        agendamento.save()
        return JsonResponse(
        ...
        return JsonResponse(
            {"success": False, "message": "Agendamento nao encontrado."}
        )
    except Exception as e:
        return JsonResponse({"success": False, "message": str(e)})

def lista_agendamentos(data):
    cliente_nome = data.get("cliente")
    agendamentos = []
    if cliente_nome:
        try:
            ...
            return JsonResponse(
                {"success": False, "message": "Cliente nao encontrado."}
            )
    return JsonResponse(
        {"success": False, "message": "Metodo invalido"}
    )
