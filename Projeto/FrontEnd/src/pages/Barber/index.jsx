...
import { Container, Context, Title, Paragraph, SubTitle, List, RegisterLink, NotificationTitle, RegisterButtons, RegisterButton, ServiceItem, ScheduleItem, DeleteButton } from "./style";
...
export function Barber() {
  ...
  const handleDeleteService = async (serviceId) => {
    try {
      const response = await api.delete(`/delete-services/`, {
        params: {
          barbeiro_id: user.id,
      ...
      if (response.data.success) {
        setServices(servicos.filter(service => service.id !== serviceId));
      ...
      if (error.response) {
        alert(`Erro: ${error.response.data.message || 'Erro ao processar requisição'}`);
      } else {
          ...
          <>
            ...
            {barbearias.filter(local => local.barbeirousuarioid === user.id).map((local) => (
              <div key={`local-${local.id}`}>
                ...
                <RegisterButton to="/edit-barbershop">Editar Barbearia</RegisterButton>
                      ...
                      <List>
                        {servicos.filter(servico => servico.local_id === local.id).map((service) => (
                          <ServiceItem key={service.id}>
                            ...
                            <DeleteButton onClick={() => handleDeleteService(service.id)}>
                              Excluir
                            </DeleteButton>
                          </ServiceItem>
                        ))}
                      </List>
                      <RegisterButton to="/services">Adicionar mais serviços</RegisterButton>
                    </>
                  ) : (
                    <>
                      <Paragraph>Nenhum serviço cadastrado.</Paragraph>
                      <RegisterButton to="/services">Cadastrar Serviços</RegisterButton>
                    </>
                  )}
                ...
                {horarios.filter(horario => horario.local_nome === local.nome_local).length > 0  ? (
                  <>
                    ...
                    <RegisterButton to="/schedule">Editar Horários</RegisterButton>
                  </>
                ) : (
                  <>
                    <Paragraph>Nenhum horário cadastrado.</Paragraph>
                    <RegisterButton to="/schedule">Cadastrar Horários</RegisterButton>
                  </>
                )}
              </div>
            ))}
