...
import { Container, Context, Title, DayContainer, StyledLink } from "./style";
...
export function Schedule() {
    ...
    function validarCampos() {
        ...
        for (const dia of diasSelecionados) {
            ...
            const camposObrigatorios = ["horaInicio", "horaFim"];

            for (const campo of camposObrigatorios) {
                if (!dados?.[campo] || !dados[campo].trim()) {
                    return `Preencha o campo "${campo}" para ${dia}.`;
                }
            }
                                ...
                                <span>
                                    ...
                                    {horarios[dia].intervalos.length > 0 &&
                                        horarios[dia].intervalos.map(
                                            (i, index) =>
                                                ` (Intervalo ${index + 1}: ${i.inicio} - ${i.duracao} min)`
                                        )}
                                </span>
                            ) : (
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: "10px" }}>
                            {diasDaSemana.map((dia) => (
                                <Button
                                    ...
                                    style={{
                                        backgroundColor: horarios[dia] ? "#4caf50" : "#ccc",
                                        color: horarios[dia] ? "white" : "black",
                                    }}
                                />
                            ))}
                        </div>
                            ...
                            {horarios[diaSelecionado]?.intervalos.map((intervalo, index) => (
                                <div key={index} style={{ border: "1px solid #ccc", padding: "8px", marginBottom: "8px" }}>
                                    <Input
                                        ...
                                        type="time"
                                        name={`intervaloInicio-${index}`}
                                        value={intervalo.inicio}
                                        onChange={(e) => handleChangeIntervalo(index, "inicio", e.target.value)}
                                        label="Início do Intervalo"
                                    />
                                    <Input
                                        placeholder="Duração do Intervalo (min)"
                                        type="number"
                                        name={`intervaloDuracao-${index}`}
